...
COVERAGE: dict[str, StepCoverage] = {
    ...
    "send_email": StepCoverage(
        ...
        note="CONNECTOR-FAMILY (SendMail, dispatcher /tasks/connector). P5 DONE: "
             ...
             "subject str, body Any, from str). The normalizer defaults "
             "connector: smtp + operation: send_email; the connector resolver "
             "auto-lifts the flat email fields into params + stamps version/"
             "operationTitle from the catalog. G10 Tier-2 DONE: decompiler "
             "minimification strips the connector envelope + unwraps params back "
             "to flat email fields (mirror of code_snippet). FORWARD-MAP FLIPPED "
             "(live-proven 2026-07-02 on 8.0/.159): the dedicated SendEmail "
             "handler (/wf/workflow/tasks/send_email, occ=0) RUNS BUT FAILS "
             "(\"Error message: 'smtp'\" -- can't resolve the configured SMTP "
             "connector), so the compiler now targets SendMail (occ=22, the "
             "connector-dispatcher path that SUCCEEDS). The smtp op takes body "
             "natively, so there is NO body->content / from->from_str rename "
             "(that was dedicated-handler-specific). Params stay Any (the smtp "
             "send_email op's per-install schema, surfaced via fsrpb find op)."),
