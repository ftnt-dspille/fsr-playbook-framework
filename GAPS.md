...
| Attribute defs | `GET /api/3/attribute_metadatas` | 🟡 (seen in Insomnia) | `AttributeMetadata.php` |
...
| List individual playbooks | `GET /api/3/workflows` or `/api/3/playbooks` | 🟡 | exact pluralization tbd from Hydra root |
...
| YAQL filters | separate language (release 7.2.0+); see Playbooks Guide pp. 218-220 | 🔴 | not yet probed; pipes via `\| yaql("$.where(...)")` — needs its own probe path |
