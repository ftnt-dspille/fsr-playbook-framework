"""Step-type catalog -- the author-facing reference for every friendly ``type:``.

The compiler already owns every data source an author needs to *discover* step
types: the resolver's friendly->canonical table, the typed-arg JSON schemas, the
packaged reference DB (``fsr_reference.db``: labels, descriptions, pitfalls,
corpus examples), and the decompiler that renders a wire snippet back into the
friendly YAML dialect. This module composes them into a small public API so
consumers (the pyfsr ``playbook steps`` / ``step-help`` CLI, an agent's
discovery tool) don't have to reach into compiler internals.
...
# verbose wire shape (manual_input's dynamicList, etc.). Known-good: each
# compiles and round-trips. Preferred over the decompiled corpus snippet when
# present.
...
def _modeled_types() -> set[str]:
    ...
    try:
        from .compiler.typed_args.schema import list_modeled_step_types
...
def step_help(name: str) -> StepHelp:
    ...
    """
    db = default_db_path()
    canonical_to_short = {v: k for k, v in SHORT_TYPE_TO_FSR.items()}

    if name in SHORT_TYPE_TO_FSR:
        short, canonical = name, SHORT_TYPE_TO_FSR[name]
    elif name in canonical_to_short:
        canonical, short = name, canonical_to_short[name]
    else:
        import difflib

        known = list(SHORT_TYPE_TO_FSR) + list(canonical_to_short)
...
def _arg_schema(short: str) -> dict[str, Any] | None:
    try:
        from .compiler.typed_args.schema import emit_step_arg_schema
...
def _example_yaml(conn: sqlite3.Connection, canonical: str, db: Any) -> str | None:
    ...
    try:
        from .compiler.decompiler import decompile_to_yaml
