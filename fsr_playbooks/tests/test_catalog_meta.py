...
def test_stamp_and_check_match(conn):
    cm.stamp_instance(conn, instance_label="dev", base_url="https://10.99.249.205")
    status, label, _ = cm.check_instance(conn, "https://10.99.249.205/")
    assert status == "ok"
    assert label == "dev"
    other, _, _ = cm.check_instance(conn, "https://10.99.249.999")
