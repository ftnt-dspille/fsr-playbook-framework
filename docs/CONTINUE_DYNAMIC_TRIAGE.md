...
## Status: DONE (all tested, 154 fsr_playbooks tests green)
All in `fsr-playbook-framework` (canonical fsr_playbooks — NOT yet re-vendored to connector).

| Layer | File | Tests |
|---|---|---|
| 0 Normalizer | `fsr_playbooks/llm/triage_normalize.py` | `test_triage_normalize.py` (11) |
| 1 Scenario registry + classifier | `fsr_playbooks/llm/triage_scenarios.py` | `test_triage_scenarios.py` (5) |
| 2 Dynamic prompt builder | `fsr_playbooks/llm/triage_prompt.py` | `test_triage_prompt.py` (5) |
| First-class SIEM tools | `fsr_playbooks/mcp_server/tools_triage.py` (`siem_search_ip/host/user`, `siem_events_for_incident`, `siem_raw_query`) | `test_siem_search_tools.py` (9) |

Registration for the SIEM tools is in 3 places (mirror this pattern for any new
tool): `fsr_playbooks/mcp_server/__init__.py` (import + `__all__`), and
`fsr_playbooks/llm/tools.py` (`SAFE_TOOLS` + `TOOL_TIERS=1`).

Also: softened the over-strong "you MUST pivot to SIEM" mandate in
`fsr_playbooks/agent/system_prompt_triage.md` to a lighter nudge (the dynamic builder
now injects scenario moves, so the blanket mandate was redundant + unreliable).

Run tests: `./.venv/bin/python -m pytest fsr_playbooks/tests/ -q` (needs the .venv).

## Build the C2 prompt to see it
```python
from fsr_playbooks.llm.triage_prompt import build_triage_prompt
import json, pathlib
raw = json.loads(pathlib.Path("fsr_playbooks/tests/fixtures/triage/alert_c2_exfil.json").read_text())
...
1. ~~**#4 Wire pre-flight into demo_hunt + connector.**~~ **DONE.** New shared
   helper `fsr_playbooks/llm/triage_preflight.py` (`triage_preflight(target|raw_record,
   ...
   `fsr_playbooks/tests/test_triage_preflight.py` (5). Wired into:
   ...
   - connector `operations.py` `_resolve_system_prompt(intent, entity, storage,
     ...
     green. NOTE: connector sees canonical fsr_playbooks via the `fsr_playbooks` symlink,
...
2. ~~**#7 Emit backend activity events to widget chat.**~~ **DONE (widget half).**
   ...
   - STILL TODO for #7: emit `activity` frames around the SIEM tool calls
     ...
     `fsr_playbooks/llm/run_turn.py`; consider emitting from the tool wrappers.
...
4. **Re-vendor to connector** when ready: `cd fsr-playbook-framework && scripts/build.sh`,
   ...
   (canonical fsr_playbooks) but NOT on the box's connector.
...
- **fsr_playbooks output summarizer wraps event lists** as
...
- `git status` shows `fsr_playbooks/mcp_server/tools_execution.py` modified with a
...
Fixtures captured: `fsr_playbooks/tests/fixtures/triage/{alert_c2_exfil,incident_smithdesktop}.json`.
