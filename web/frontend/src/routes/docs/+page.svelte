      ...
      <div class="mt-10 space-y-12 text-base leading-relaxed text-[var(--text-default)]">
        <section id="overview">
          ...
          <ol class="list-decimal space-y-1 pl-6 text-[var(--text-muted)]">
            ...
            <li>A run viewer that streams the CLI's output and rebuilds the runtime <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">vars</code> tree.</li>
          </ol>
          <p class="mt-3 text-[var(--text-muted)]">
            Everything sits on top of the existing <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">fsrpb</code> CLI and <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">store/fsr_reference.db</code> — no FortiSOAR knowledge is baked into the web app itself.
          </p>
        </section>
...
└────────────┬──────────────────────┬─────────────────────┘
   ...
   └────────────────────┘  └─────────────────────────────┘`}</pre>
        ...
        <section id="design-tab">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">Design tab</h2>
          ...
          <ul class="list-disc space-y-2 pl-6 text-[var(--text-muted)]">
            <li><span class="font-semibold">Editor</span> — every keystroke triggers a 400ms debounced <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">POST /api/yaml/validate</code>. Errors and warnings render as Monaco squigglies and in the Diagnostics drawer.</li>
        ...
        <section id="autocomplete">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">Autocomplete</h2>
          <p class="mb-3">Monaco completions, scoped by the YAML key:</p>
          <ul class="list-disc space-y-2 pl-6 text-[var(--text-muted)]">
            <li><code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">type:</code> — all 15 short step types. Picking one inserts a snippet scaffold; press Tab to jump between fields.</li>
            <li><code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">connector:</code> — fuzzy-searches all 714 connectors from the reference DB.</li>
            <li><code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">operation:</code> — looks at the <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">connector:</code> line above and lists only that connector's ops.</li>
          </ul>
        </section>

        <section id="examples">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">Examples menu</h2>
          <p>
            The <span class="font-semibold">Examples ▾</span> dropdown lists every fixture in
            <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">FSRPlaybookYaml/examples/*.yaml</code>.
            Click one to load it into the editor. <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">.test.yaml</code> sidecars are excluded.
          </p>
        </section>

        <section id="validate-compile">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">Validate vs Compile</h2>
          ...
          <ul class="list-disc space-y-2 pl-6 text-[var(--text-muted)]">
            ...
            <li><span class="font-semibold">Compile</span> (button): same pipeline, but on success also emits the FortiSOAR <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">WorkflowCollection</code> JSON — what would get POSTed to the appliance.</li>
          </ul>
        </section>

        <section id="push-run">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">Push &amp; Run — talking to FortiSOAR</h2>
          <p class="mb-3">Both buttons subprocess the existing <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">fsrpb</code> CLI.</p>
        ...
        <section id="run-tab">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">Run tab — logs and env viewer</h2>
          <ul class="list-disc space-y-2 pl-6 text-[var(--text-muted)]">
            <li>Mirrors the Run log with a full-window view.</li>
            <li>After a run finishes with a parsed <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">task_id</code>, the env rail auto-loads.</li>
            <li>Or paste any past PK / task UUID; backend subprocesses <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">python -m cli env &lt;pk&gt;</code> which calls <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">GET /api/wf/api/workflows/&lt;pk&gt;/?step_detail=true</code> and rebuilds the Jinja context.</li>
            <li>The <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">Step_Name</code> form is the canonical Jinja key — display name with spaces→underscores.</li>
          </ul>
        </section>
        ...
        <section id="tools">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">The 14 tools</h2>
          <p class="mb-3 text-[var(--text-muted)]">
            Read-only by design. <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">run_op</code>, <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">push</code>, and <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">run_playbook</code> are <span class="font-semibold">not</span> available to the chat.
          </p>
        ...
        <section id="health">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">Status pills</h2>
          ...
          <ul class="list-disc space-y-2 pl-6 text-[var(--text-muted)]">
            ...
            <li><span class="font-semibold">LLM</span> — green if <code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">ANTHROPIC_API_KEY</code> is set.</li>
          </ul>
        </section>
        ...
        <section id="env">
          ...
          <ul class="list-disc space-y-2 pl-6 text-[var(--text-muted)]">
            <li><code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">ANTHROPIC_API_KEY</code> in the shell where you run uvicorn.</li>
            <li><code class="rounded bg-[var(--bg-elevated)] px-1 text-sm">.env</code> in the FSRPlaybookYaml repo root.</li>
          </ul>
...
FSR_ALLOW_E2E=true`}</pre>
        ...
        <section id="tests">
          ...
          <pre class="overflow-auto rounded border border-[var(--border-soft)] bg-[var(--bg-canvas)] p-4 font-mono text-xs leading-tight text-[var(--text-muted)]">{`# Web backend (20 tests)
...
cd FSRPlaybookYaml && python3 -m pytest python/tests/ -q`}</pre>
        ...
        <section id="roadmap">
          <h2 class="mb-3 text-xl font-semibold text-[var(--text-default)]">Roadmap</h2>
          <ul class="list-disc space-y-2 pl-6 text-[var(--text-muted)]">
        ...
        <footer class="mt-12 border-t border-[var(--border-soft)] pt-4 text-xs text-[var(--text-faint)]">
          ...
          <code class="rounded bg-[var(--bg-elevated)] px-1">CHAT_APP_PLAN.md</code>, and the implementation under <code class="rounded bg-[var(--bg-elevated)] px-1">web/backend</code> and <code class="rounded bg-[var(--bg-elevated)] px-1">web/frontend</code>.
        </footer>
      </div>
    </div>
  </article>
